...
void joc()
{
	...
	//Buttons
	Button botoCatala = Button(100, 120, "Cat");
	Button botoCastella = Button(100, 170, "Cast");
	Button botoAngles = Button(100, 220, "Ang");
	...
	do
	{
		...
		switch (m_estatActual)
		{
			...
			case PLAYING:
				...
				if (!pausarContador) {
					timer += deltaTime; // Va sumant els segons
				...
				if (drawTimer) {
					fontNegra.draw(200, 50, NFont::Scale(0.85f), "%d", (int)timer);
				}
				if (timer > 10) {
					drawTimer = false;
			...
			case GAME_OVER:
				...
				if (timerGameOver > 5.0f) {
					fonsFinalPartida.draw(800, 800);
					//fontNegra.draw(200, 50, NFont::Scale(0.85f), "TIME'S OVER");
